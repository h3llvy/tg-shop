...
import SendGiftModal from '../components/SendGiftModal.vue'
...
const handleSend = (gift: IUserGift) => {
...
onMounted(async () => {
  try {
    ...
    if (!initData) {
      error.value = 'Ошибка авторизации: отсутствуют данные инициализации'
      return
    }
...
<template>
  <div class="min-h-screen bg-bg-primary-light dark:bg-bg-primary-dark">
    ...
    <SendGiftModal 
      v-if="showSendModal && selectedGift"
